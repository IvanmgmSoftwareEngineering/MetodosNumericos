               ...
               hold on;
                ...
                % grafica que la funcion f(x)
                 ...
                 % misma grafica
                  ...
                  %Dibujo la comparaci?n entre f(x) y p(x), es decir,
                  %|f(x)-p(x)|
                  X2=linspace(a,b,1000);
                  for i=1:1000
                        I1(i)=subs(p,x,X2(i));
                        F1(i)=subs(f,x,X2(i));
                        compara(i)=abs(I1(i)-F1(i));
                  end
                  subplot(1,3,2);
                   plot(X2,compara,'MarkerEdgeColor',[0.2 0.2 0.2],'LineWidth',2);
                   title('|f(x)-p(x)|');
                   xlabel('x');
                   if(n==4)
                    axis([a b 0 0.03]);
